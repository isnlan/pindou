...
export const editorHelpArticles: EditorHelpArticle[] = [
  ...
  },
  {
    ...
    keywords: ["图片", "定位", "缩放", "预览", "拖动", "生成", "颜色数量", "限色"],
    sections: [
      {
        title: "操作方式",
        items: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "图片定位和缩放只有点击“生成图纸”后才会按当前预览状态写入画布。",
        ],
      },
      {
        title: "颜色数量",
        items: [
          "颜色数量会从高级颜色中已启用的候选色里自动挑选，不会使用被禁用的颜色。",
          "减少颜色时会优先合并视觉上相近的颜色，并把低占比色并入累计格子数更多的主色。",
          "滑杆停顿片刻后会直接重新生成正式画布，已有手工修改会被覆盖，但可以一步撤销。",
          "拖到最大值表示不限色；透明区域、纯色图片等情况下，实际用色可能少于目标数量。",
        ],
      },
    ],
  },
